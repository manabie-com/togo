...
func (s httpServer) AddTask(c echo.Context) error {
	...
	task, err := s.taskUsecase.Create(c.Request().Context(), &domain.Task{
...
func (s httpServer) GetTasks(c echo.Context) error {
	...
	tasks, err := s.taskUsecase.FindByUserID(c.Request().Context(), currentUser.ID)
	if err != nil {
		return err
	}
...
func (s httpServer) UpdateTask(c echo.Context) error {
	...
	task, err := s.taskUsecase.Update(c.Request().Context(),
		&domain.Task{
			ID:     u.ID,
			UserID: currentUser.ID,
		},
