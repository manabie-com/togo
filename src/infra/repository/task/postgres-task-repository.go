...
func (this *TaskRepository) Count(filter interface{}) (int, error) {
	var count int
	result := this.db.PConn.Model(&task.Task{}).
		Where(filter).
		Count(&count)

	if result.Error != nil {
		return 0, this.errorFactory.InternalServerError(src.CREATE_TASK_ERROR, result.Error)
	}
