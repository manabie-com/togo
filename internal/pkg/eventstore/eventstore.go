...
// EventStore implements an eventhorizon.EventStore for PostgreSQL.
...
type AggregateEvent struct {
	...
	AggregateType eventhorizon.AggregateType `sql:"aggregate_type,type:varchar(250)"`
	EventType     eventhorizon.EventType     `sql:"event_type,type:varchar(250)"`
	RawData       json.RawMessage            `sql:"data,type:jsonb"`
	Timestamp     time.Time                  `sql:"timestamp"`
	Version       int                        `sql:"version"`
	Context       map[string]interface{}     `sql:"context"`
	data          eventhorizon.EventData     `sql:"-"`
}
...
func (s *EventStore) newDBEvent(ctx context.Context, event eventhorizon.Event) (*AggregateEvent, error) {
	ns := eventhorizon.NamespaceFromContext(ctx)

	// Marshal event data if there is any.
	raw, err := s.encoder.Marshal(event.Data())
	if err != nil {
		return nil, eventhorizon.EventStoreError{
			BaseErr:   err,
	...
	return &AggregateEvent{
		...
		Context:       eventhorizon.MarshalContext(ctx),
		Namespace:     ns,
	}, nil
}
...
func (s *EventStore) Save(ctx context.Context, events []eventhorizon.Event, originalVersion int) error {
	ns := eventhorizon.NamespaceFromContext(ctx)

	if len(events) == 0 {
		return eventhorizon.EventStoreError{
			Err:       eventhorizon.ErrNoEventsToAppend,
			Namespace: ns,
		}
	}
	...
	for i, event := range events {
		...
		if event.AggregateID() != aggregateID {
			return eventhorizon.EventStoreError{
				Err:       eventhorizon.ErrInvalidEvent,
				Namespace: ns,
			}
		}
		...
		if event.Version() != version+1 {
			return eventhorizon.EventStoreError{
				Err:       eventhorizon.ErrIncorrectEventVersion,
				Namespace: ns,
			}
		}
...
func (s *EventStore) Load(ctx context.Context, id uuid.UUID) ([]eventhorizon.Event, error) {
	ns := eventhorizon.NamespaceFromContext(ctx)
...
func (s *EventStore) Replace(ctx context.Context, event eventhorizon.Event) error {
	ns := eventhorizon.NamespaceFromContext(ctx)
	...
	if err != nil {
		...
		return eventhorizon.ErrAggregateNotFound
	}
	...
	if err == pg.ErrNoRows {
		return eventhorizon.ErrInvalidEvent
	} else if err != nil {
		return eventhorizon.EventStoreError{
			BaseErr:   err,
			Err:       err,
...
func (s *EventStore) RenameEvent(ctx context.Context, from, to eventhorizon.EventType) error {
	ns := eventhorizon.NamespaceFromContext(ctx)
