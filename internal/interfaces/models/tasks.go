...
var TaskWhere = struct {
	...
	ID:          whereHelperint{field: "\"togo\".\"tasks\".\"id\""},
	Title:       whereHelperstring{field: "\"togo\".\"tasks\".\"title\""},
	Description: whereHelpernull_String{field: "\"togo\".\"tasks\".\"description\""},
	Priority:    whereHelperint{field: "\"togo\".\"tasks\".\"priority\""},
	IsDone:      whereHelperbool{field: "\"togo\".\"tasks\".\"is_done\""},
	UserID:      whereHelperint{field: "\"togo\".\"tasks\".\"user_id\""},
	CreatedAt:   whereHelpertime_Time{field: "\"togo\".\"tasks\".\"created_at\""},
	UpdatedAt:   whereHelpertime_Time{field: "\"togo\".\"tasks\".\"updated_at\""},
	DeletedAt:   whereHelpernull_Time{field: "\"togo\".\"tasks\".\"deleted_at\""},
}
...
var (
	taskAllColumns            = []string{"id", "title", "description", "priority", "is_done", "user_id", "created_at", "updated_at", "deleted_at"}
	taskColumnsWithoutDefault = []string{"title", "user_id"}
	taskColumnsWithDefault    = []string{"id", "description", "priority", "is_done", "created_at", "updated_at", "deleted_at"}
...
func (taskL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTask interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`togo.users`),
		qm.WhereIn(`togo.users.id in ?`, args...),
		qmhelper.WhereIsNull(`togo.users.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Task) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	...
	updateQuery := fmt.Sprintf(
		"UPDATE \"togo\".\"tasks\" SET %s WHERE %s",
...
func Tasks(mods ...qm.QueryMod) taskQuery {
	mods = append(mods, qm.From("\"togo\".\"tasks\""), qmhelper.WhereIsNull("\"togo\".\"tasks\".\"deleted_at\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"togo\".\"tasks\".*"})
	}
...
func FindTask(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Task, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"togo\".\"tasks\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *Task) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"togo\".\"tasks\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"togo\".\"tasks\" %sDEFAULT VALUES%s"
		}
...
func (o *Task) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"togo\".\"tasks\" SET %s WHERE %s",
...
func (o TaskSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"togo\".\"tasks\" SET %s WHERE %s",
...
func (o *Task) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"togo\".\"tasks\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Task) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = "DELETE FROM \"togo\".\"tasks\" WHERE \"id\"=$1"
	} else {
		currTime := time.Now().In(boil.GetLocation())
		o.DeletedAt = null.TimeFrom(currTime)
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"togo\".\"tasks\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(taskType, taskMapping, append(wl, taskPrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q taskQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o TaskSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		sql = "DELETE FROM \"togo\".\"tasks\" WHERE " +
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"togo\".\"tasks\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, taskPrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func (o *TaskSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"togo\".\"tasks\".* FROM \"togo\".\"tasks\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, taskPrimaryKeyColumns, len(*o)) +
...
func TaskExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"togo\".\"tasks\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
